...
**トーナメント(勝ち抜き/シングルエリミネーション)**: 人数が2の累乗でない場合は自動的に不戦勝(バイ)が配置され、1回戦で解消されます。勝者は自動的に次の対局へ進出。決勝が引き分けた場合はその対局だけやり直しになります(トーナメントは必ず1人の優勝者が決まる必要があるため)。

## CPU練習モード

`practice.html` からFirebase不要でいつでも練習できます。あなたが先手、CPUが後手で固定です。

- **かんたん**: 完全ランダムに指すCPU。ルールを覚えたい人向け
- **ふつう**: 2手先まで読むミニマックス探索
- **つよい**: 4手先まで読む本気モード

盤がとても小さい(3×4)ため、4手読みでも計算は一瞬です。同じ強さの中でも最善手が複数あるときはランダムに選ぶので、毎回同じ展開にはなりません。
